...
import org.junit._
import Assert._
private object FinalValLoggerTest extends DeployTimeConfigure {
  private final val (logger, formatter) = ZeroLoggerFactory.newLogger(this)
  import formatter._
...
class FinalValLoggerTest {
  ...
  @Test
  def test = {
    print("zero-log spends "); print(0 until 5 map { FinalValLoggerTest.doTest(math.random, _) } reduce { math.min(_, _) } toString()); println(" nanoseconds per log.")
  }
