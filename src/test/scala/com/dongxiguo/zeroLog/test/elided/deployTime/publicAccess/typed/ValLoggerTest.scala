...
package deployTime.publicAccess.typed

import com.dongxiguo.zeroLog.Logger
import com.dongxiguo.zeroLog.deployTime.DeployTimeConfigure
import org.junit._
import Assert._
private object ValLoggerTest extends DeployTimeConfigure {
  val (logger: Logger, formatter) = ZeroLoggerFactory.newLogger(this)
  import formatter._
...
class ValLoggerTest {
  ...
  @Test
  def test = {
    print("zero-log spends "); print(0 until 5 map { ValLoggerTest.doTest(math.random, _) } reduce { math.min(_, _) } toString()); println(" nanoseconds per log.")
  }
